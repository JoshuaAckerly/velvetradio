    ...
    return (
        <AdminLayout title="Shows">
            <div className="mb-6 flex items-center justify-between">
                ...
                <button
                    onClick={() => router.visit('/admin/shows/create')}
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-[#3a3a3a] text-left text-[var(--muted-foreground,#888)]">
                            ...
                            <th className="pr-4 pb-2">Hosts</th>
                            <th className="pr-4 pb-2">Episodes</th>
                            <th className="pr-4 pb-2">Active</th>
                        ...
                        {shows.map((show) => (
                            <tr key={show.id} className="border-b border-[#2a2a2a]">
                                ...
                                <td className="py-3 pr-4">
                                    <span
                                        className={`rounded px-2 py-0.5 text-xs font-semibold ${show.active ? 'bg-green-900 text-green-300' : 'bg-gray-700 text-gray-400'}`}
                                    >
                                ...
                                <td className="py-3 text-right">
                                    <button
                                        onClick={() => router.visit(`/admin/shows/${show.id}/edit`)}
                                        className="mr-3 text-[#4a3d5c] hover:text-white"
                                    >
                                        Edit
                                    </button>
                                    <button onClick={() => destroy(show.id)} className="text-red-500 hover:text-red-300">
                                        Delete
                                    </button>
                                </td>
                            </tr>
