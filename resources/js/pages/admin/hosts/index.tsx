    ...
    return (
        <AdminLayout title="Hosts">
            <div className="mb-6 flex items-center justify-between">
                ...
                <button
                    onClick={() => router.visit('/admin/hosts/create')}
                            ...
                            <tr key={host.id} className="border-b border-[#2a2a2a]">
                                <td className="py-3 pr-4 font-medium">
                                    <div className="flex items-center gap-2">
                                        {host.avatar && <img src={host.avatar} alt={host.name} className="h-7 w-7 rounded-full object-cover" />}
                                        {host.name}
                                    </div>
                                </td>
                                <td className="py-3 pr-4 text-[var(--muted-foreground,#888)]">{host.show?.title ?? '—'}</td>
                                <td className="max-w-xs truncate py-3 pr-4 text-[var(--muted-foreground,#888)]">{host.bio ?? '—'}</td>
                                <td className="py-3 text-right">
                                    <button
                                        onClick={() => router.visit(`/admin/hosts/${host.id}/edit`)}
                                        className="mr-3 text-[#4a3d5c] hover:text-white"
                                    >
                                        Edit
                                    </button>
                                    <button onClick={() => destroy(host.id)} className="text-red-500 hover:text-red-300">
                                        Delete
                                    </button>
                                </td>
                            </tr>
